...
export default function ReachModal({ open, setOpen }) {
  ...
  async function handleOtp() {
    const res = await mutateClient("/otp/request", {
      body: { phone: getLocalStorage("student")?.parent_phone },
    });
    if (res.status) {
      setOtp(true);
    }
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl rounded-lg bg-white p-8 shadow-lg">
          ...
          <div
            ...
            className="bg-background border-primary-800 flex w-full flex-wrap items-center justify-center gap-[24px] rounded-lg border p-2 py-3 font-bold md:flex-nowrap md:py-[32px] md:text-[24px]"
          >
            <img src="/assets/Whatsapp.svg" className="h-[48px] w-[48px]" />
