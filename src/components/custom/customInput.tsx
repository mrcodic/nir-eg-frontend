      ...
      render={({ field }) => (
        <div className={`w-full flex  flex-col mt-12! relative ${className}`}>
          ...
          <div className=" relative">
            {/* <div className="p-2 flex items-center gap-2 border-b border-primary-700 text-[#454545]"> */}
            <FormControl>
              <Input
                type={changeHidePassword === false ? type : "text"}
                placeholder={""}
                className="text-[14px]  font-medium placeholder:text-[#454545]  text-[#454545] w-full border-none outline-hidden focus:border-none disabled:cursor-not-allowed disabled:opacity-70"
                {...field}
                id={name}
            ...
            <FormLabel
              htmlFor={name}
              className="flex text-custom-brown mb-2 text-[14px] gap-4 absolute font-medium -top-4 text-xs  right-0 cursor-text peer-focus:text-xs peer-focus:-top-4 transition-all peer-focus:text-custom-brown peer-placeholder-shown:top-1 peer-placeholder-shown:text-sm"
            >
              {iconSrc && (
