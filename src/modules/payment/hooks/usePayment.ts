...
function openPaymentWindow(url: string) {
  ...
  const normalizedUrl = normalizePaymentUrl(url);

  if (typeof window !== "undefined" && window.electron?.openPayment) {
    window.electron.openPayment(normalizedUrl);
    return;
  }
    ...
    if (
      ...
      try {
        ...
        if (response?.payment_url) {
          ...
          openPaymentWindow(response.payment_url);
